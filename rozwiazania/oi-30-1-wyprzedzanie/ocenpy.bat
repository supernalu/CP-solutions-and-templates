...
echo.
echo Sposob uzycia:
echo   ocenpy.bat id_zadania [ nr_testu ]
echo.
goto end
...
set I=%2
set N=%3
%PYTHON_CMD% --version > NUL 2>NUL || goto err_pyexe
%PYTHON_CMD% -c "import sys; exit(0 if sys.version.startswith('3') else 1)" > NUL 2> NUL || goto err_py3
if not exist %I%.py goto err_py
if not exist %DIR%IN\%I%%N%.in goto err_in
if exist %I%.in  del %I%.in
if exist %I%.out del %I%.out
copy %DIR%IN\%I%%N%.in %I%.in > nul
break on
%DIR%BIN\timer.exe %PYTHON_CMD% %I%.py
echo.
...
break on
if "%W32%"=="1" goto w32
%DIR%BIN\timer.exe %PYTHON_CMD% %I%.py < %DIR%IN\%I%%N%.in > %I%.out
goto run_ok
:w32
%DIR%BIN\timer.exe %DIR%BIN\start.exe /w %PYTHON_CMD% %I%.py < %DIR%IN\%I%%N%.in > %I%.out
:run_ok
type timer.out
...
FOR %%i IN (%T%) DO call ocenpy.bat _chk_ %I% %%i
...
:gen_all
FOR %%i IN (%T%) DO call ocenpy.bat _gen_ %I% %%i
goto end
...
:err_pyexe
echo * Brak Pythona. Jezeli masz niestandardowa instalacje, zmien zmienna PYTHON_CMD w czwartej linii tego skryptu. *
goto end

:err_py3
echo * Zla wersja Pythona (powinna byc 3.x). Jezeli masz niestandardowa instalacje, zmien zmienna PYTHON_CMD w czwartej linii tego skryptu. *
goto end

:err_py
echo * Brak pliku %I%.py *
goto end
