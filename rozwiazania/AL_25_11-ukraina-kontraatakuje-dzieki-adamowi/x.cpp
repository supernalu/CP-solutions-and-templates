...
void Rootify(int root) {
    ...
    for (int pow = 1; pow < 30; pow++) {
        for (int i = 1; i <= n; i++) {
            ...
            objects[i][pow] = combine(objects[i][pow-1], objects[ancestor[i][pow-1]][pow-1]);
...
vector<int> lca(int a, int b) {
    ...
    for (int i = 29; i >= 0; i--)
        if (tmp >= (1 << i)) {
            tmp -= (1 << i);
            res = combine(res, objects[a][i]);
            a = ancestor[a][i];
        }
    if (a == b) {
        res = combine(res, objects[a][0]);
        return res;
    }
    for (int i = 29; i >= 0; i--)
        if (ancestor[a][i] != ancestor[b][i]) {
            res = combine(res, objects[a][i]); res = combine(res, objects[b][i]);
            a = ancestor[a][i]; b = ancestor[b][i];
        }
    res = combine(res, objects[a][1]);
    res = combine(res, objects[b][0]);
    return res;
}
int main() {
